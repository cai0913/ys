(function() {
    ...
    var serverUrl = '../php/process.php';
    var list = $('.jslist');
    var edit = $('.edit');
    var opt = [
        '<a href="#" class="jslist-edit">修改</a>',
        '<a href="#" class="jslist-del">删除</a>'
    ];
    var jslist = new Jslist(list.find('table'));
    var editor = new Editor(edit);
    $.getJSON(serverUrl, function(data) {
        for (var i in data) {
            jslist.append(data[i], opt);
        }
    });
    list.on('click', '.jslist-del', function() {
    ...
    list.on('click', '.jslist-edit', function() {
        var i = $(this).parents('tr').index();
        var data = jslist.getData(i);
    ...
    $('.search').click(function() {
        jslist.search($.trim($('.search-input').val()));
    });
})();